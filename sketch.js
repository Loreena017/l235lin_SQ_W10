...
let previousGameState = STATE_PLAY;
...
function preload() {
  ...
  wolf = loadImage("assets/images/wolf-character.png");
  grassfloor = loadImage("assets/images/grass-background.jpg");
  villager = loadImage("assets/images/villager_sprite.png");
  appleImage = loadImage("assets/images/apple.png");

  shootSound     = loadSound("assets/sounds/gun_sound.mp3");
...
// ------------------------------------------------------------
// updatePowerups()
// Decreases any active temporary boosts.
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// spawnApple()
// Places the apple collectible at a safe spot in the world.
// ------------------------------------------------------------
function spawnApple() {
  let attempts = 0;
  ...
  while (attempts < 20) {
    ...
    if (dist(candidate.x, candidate.y, player.x, player.y) > 220) {
      apple = candidate;
      return;
    }
...
// ------------------------------------------------------------
// updateAppleState()
// Handles apple respawn timing.
// ------------------------------------------------------------
function updateAppleState() {
...
// ------------------------------------------------------------
// checkAppleCollection()
// Gives the player a speed boost when they collect the apple.
// ------------------------------------------------------------
function checkAppleCollection() {
  if (!apple) return;

  let d = dist(player.x, player.y, apple.x, apple.y);
  if (d < player.r + apple.size * 0.4) {
    player.powerupTimer = POWERUP_DURATION;
    apple = null;
    appleRespawnTimer = APPLE_RESPAWN_DELAY;
  }
}

// ------------------------------------------------------------
// drawApple()
// Draws the collectible apple in world space.
// ------------------------------------------------------------
function drawApple() {
  if (!apple) return;
  ...
  push();
  imageMode(CENTER);
  let bob = sin(frameCount * 0.08) * 3;
  image(appleImage, apple.x, apple.y + bob, apple.size, apple.size);
  pop();
}
...
function handleInput() {
  ...
  let moveSpeed = BASE_PLAYER_SPEED;
  if (player.powerupTimer > 0) {
    moveSpeed += POWERUP_SPEED_BOOST;
  }

  if (keyIsDown(87)) { player.y -= moveSpeed; player.direction = "up"; player.isMoving = true; }
  if (keyIsDown(83)) { player.y += moveSpeed; player.direction = "down"; player.isMoving = true; }
  if (keyIsDown(65)) { player.x -= moveSpeed; player.direction = "left"; player.isMoving = true; }
  if (keyIsDown(68)) { player.x += moveSpeed; player.direction = "right"; player.isMoving = true; }

  player.x = constrain(player.x, player.hw, WORLD_W - player.hw);
  player.y = constrain(player.y, player.hh, WORLD_H - player.hh);
  ...
  if (keyIsDown(UP_ARROW)) {
    shootDirection = DIR.up;
    player.direction = "up";
  } else if (keyIsDown(DOWN_ARROW)) {
    shootDirection = DIR.down;
  ...
  if (shootDirection && player.shootTimer === 0) {
    bullets.push({
      x: player.x + shootDirection.x * (player.hw + 4),
      y: player.y + shootDirection.y * (player.hh + 4),
      vx: shootDirection.x * BULLET_SPEED,
      vy: shootDirection.y * BULLET_SPEED,
    });
...
function drawBoss() {
  ...
  // Draw wolf sprite
  image(wolf, boss.x, boss.y, size, size);
...
function drawEnemies() {
  for (let i = 0; i < enemies.length; i++) {
    ...
    push();
    imageMode(CENTER);
    image(wolf, e.x, e.y, e.r * 3, e.r * 3);
    pop();
  }
}
...
function drawCharacter() {
  ...
  push();
  imageMode(CENTER);
  image(villager, player.x, player.y, dw, dh, sx, sy, SPRITE.frameWidth, SPRITE.frameHeight);
  pop();
}
...
function drawMinimap() {
  ...
  if (apple) {
    fill(255, 220, 80);
    let ap = worldToMap(apple.x, apple.y);
    ellipse(ap.x, ap.y, 7);
  }
...
function drawHUD() {
  ...
  textAlign(LEFT);
  textFont("monospace");
  text("Move: WASD   Shoot: Arrow keys   B: Boss fight", 16, 24);
  ...
  if (player.powerupTimer > 0) {
    ...
    textSize(11);
    textAlign(LEFT);
    text("Speed boost active!", 16, 48);

    let boostBarW = 140;
    let boostBarH = 8;
    let boostBarX = 16;
    let boostBarY = 58;
    let boostFillW = map(player.powerupTimer, 0, POWERUP_DURATION, 0, boostBarW);

    fill(40);
    rect(boostBarX, boostBarY, boostBarW, boostBarH, 4);
    fill(255, 220, 80);
    rect(boostBarX, boostBarY, boostFillW, boostBarH, 4);
  }
...
function keyPressed() {
  if (key === " " || keyCode === 32) {
    ...
    } else if (gameState === STATE_PLAY || gameState === STATE_BOSS) {
  ...
  // R — restart
  if ((key === "r" || key === "R") && gameState !== STATE_PLAY && gameState !== STATE_BOSS && gameState !== STATE_PAUSED) {
